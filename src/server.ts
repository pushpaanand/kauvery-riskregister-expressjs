...
app.get('/api/ai/ping', async (_req, res) => {
  try {
    ...
    try {
      ...
      const text: string = (resp as any)?.choices?.[0]?.message?.content || '';
      ...
      try {
        if (/\.openai\.azure\.com\/?$/i.test(normalizedEndpoint)) {
          ...
          const text2: string = (resp2 as any)?.choices?.[0]?.message?.content || '';
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (!name || !['user','manager','admin'].includes(role)) return res.status(400).json({ error: 'Invalid name or role' });
...
app.get('/api/incidents', async (req, res) => {
  try {
    ...
    const riskNo = req.query.riskNo as string | undefined;
...
app.post('/api/users', async (req, res) => {
  try {
    ...
    if (!name) return res.status(400).json({ error: 'Name is required' });
    if (!['user','manager','admin','unit_head'].includes(role)) return res.status(400).json({ error: 'Invalid role' });
    if (employeeId) {
      const six = /^[0-9]{6}$/;
      const full = /^[0-9]{6}@kauveryhospital\.com$/i;
      if (six.test(employeeId)) employeeId = `${employeeId}@kauveryhospital.com`;
      else if (full.test(employeeId)) { const digits = employeeId.substring(0, 6); employeeId = `${digits}@kauveryhospital.com`; }
      else return res.status(400).json({ error: 'Employee ID must be 6 digits or 6digits@kauveryhospital.com' });
    }
    const pool = await getPool();
    let departmentId: string | null = null;
    if (role !== 'admin' && role !== 'unit_head') {
      ...
      if (depSel.recordset.length) departmentId = depSel.recordset[0].DepartmentId;
      else {
    ...
    if (employeeId) {
      ...
      if (dup.recordset.length) return res.status(409).json({ error: 'Employee ID already exists' });
    }
    const rq = pool.request();
    rq.input('Name', name);
...
app.put('/api/users/:id', async (req, res) => {
  try {
    ...
    if (nextRole === 'admin' || nextRole === 'unit_head') {
      ...
      if (departmentName === '') return res.status(400).json({ error: 'Department is required for user/manager' });
